...
@pytest.fixture
def mock_sio():
    with patch('app.services.game_list_events.sio.sio_game_list.emit', new_callable=AsyncMock) as mock_emit:
        yield mock_emit
...
async def test_emit_game_list_with_games(mock_db_session, mock_sio):
    mock_game = Game(id=1, name="Test Game", status=GameStatus.LOBBY, players=[], max_players=4)
    mock_db_session.query.return_value.filter.return_value.all.return_value = [mock_game]
