...
def reset_player_lock():
    ...
    PlayerLock._instance = None  # Resetea la instancia singleton para asegurar un estado limpio
...
def test_lock_player_context_manager_raises_error_if_locked(player_lock, player_data):
    player_id = player_data["player_id"]
    action = player_data["action"]
