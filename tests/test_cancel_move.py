...
async def test_cancel_move_success(db_session):
    ...
    db_session.query(ParallelBoard).filter_by.return_value.order_by.return_value.first.return_value = parallel_board
    db_session.query(ParallelBoard).filter_by.return_value.all.return_value = [parallel_board]
    ...
    db_session.query(SquarePiece).filter.return_value.all.return_value = [square_piece]
    ...
    db_session.query(CardMove).filter.return_value.first.return_value = card_move

    with patch('app.services.board.game_events.emit_board', new_callable=AsyncMock) as mock_emit_board, \
         patch('app.services.board.game_events.emit_opponents_total_mov_cards', new_callable=AsyncMock) as mock_emit_opponents_total_mov_cards, \
         patch('app.services.board.game_events.emit_cards', new_callable=AsyncMock) as mock_emit_cards, \
         patch('app.services.board.game_events.emit_found_figures', new_callable=AsyncMock) as mock_emit_found_figures:
        ...
        mock_emit_cards.assert_awaited_once_with(game_id, player_id, db_session)
...
async def test_cancel_move_no_board_state(db_session):
    ...
    db_session.query(ParallelBoard).filter_by.return_value.order_by.return_value.first.return_value = None
...
async def test_cancel_move_sqlalchemy_error(db_session):
    ...
    db_session.query(ParallelBoard).filter_by.return_value.order_by.return_value.first.side_effect = SQLAlchemyError
